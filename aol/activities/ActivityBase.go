...
func (self *ActivityBase) GetSearchKeyword() (keyword, filter string) {
	...
	fmt.Println("[INFO] filter: " + filter + ", keyword: " + keyword)
...
func (self *ActivityBase) IsAvailableMailActionByName(name, dual string) bool {
	var value string
	errName := chromedp.Run(self.Tasks, chromedp.EvaluateAsDevTools(`$x('//li[@title="`+name+`"]')[0].type`, &value))
	errDual := chromedp.Run(self.Tasks, chromedp.EvaluateAsDevTools(`$x('//li[@title="`+dual+`"]')[0].type`, &value))
